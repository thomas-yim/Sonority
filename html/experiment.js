...
let expInfo = {'participant': '', 'ID Number': ''};
...
flowScheduler.add(surveyInstructionsRoutineBegin());
flowScheduler.add(surveyInstructionsRoutineEachFrame());
flowScheduler.add(surveyInstructionsRoutineEnd());
flowScheduler.add(ageSurveyRoutineBegin());
flowScheduler.add(ageSurveyRoutineEachFrame());
flowScheduler.add(ageSurveyRoutineEnd());
flowScheduler.add(genderSurveyRoutineBegin());
flowScheduler.add(genderSurveyRoutineEachFrame());
flowScheduler.add(genderSurveyRoutineEnd());
flowScheduler.add(nativeLangSurveyRoutineBegin());
flowScheduler.add(nativeLangSurveyRoutineEachFrame());
flowScheduler.add(nativeLangSurveyRoutineEnd());
flowScheduler.add(otherLangSurveyRoutineBegin());
flowScheduler.add(otherLangSurveyRoutineEachFrame());
flowScheduler.add(otherLangSurveyRoutineEnd());
...
var xText;
var jText;
var surveyInstructionsClock;
var surveyInstrText;
var skipSurveyInstr;
var ageSurveyClock;
var age;
var ageInstr;
var genderSurveyClock;
var gender;
var genderInstr;
var nativeLangSurveyClock;
var nativeLang;
var nativeLangInstr;
var otherLangSurveyClock;
var otherLang;
var otherLangInstr;
var thankuserClock;
var byeText;
...
function experimentInit() {
  ...
  surveyInstructionsClock = new util.Clock();
  ...
  skipSurveyInstr = new core.Keyboard({psychoJS: psychoJS, clock: new util.Clock(), waitForStart: true});
  
  // Initialize components for Routine "ageSurvey"
  ageSurveyClock = new util.Clock();
  ...
  genderSurveyClock = new util.Clock();
  ...
  genderInstr = new visual.TextStim({
    win: psychoJS.window,
    name: 'genderInstr',
  ...
  nativeLangSurveyClock = new util.Clock();
  nativeLang = new visual.TextStim({
    win: psychoJS.window,
    name: 'nativeLang',
    text: '',
    font: 'Arial',
  ...
  nativeLangInstr = new visual.TextStim({
    win: psychoJS.window,
    name: 'nativeLangInstr',
  ...
  otherLangSurveyClock = new util.Clock();
  otherLang = new visual.TextStim({
    win: psychoJS.window,
    name: 'otherLang',
    text: '',
    font: 'Arial',
  ...
  otherLangInstr = new visual.TextStim({
    win: psychoJS.window,
    name: 'otherLangInstr',
...
function train1feedbackRoutineBegin(trials) {
  return function () {
    ...
    for (const thisComponent of train1feedbackComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function test1RoutineEachFrame(trials) {
  return function () {
    ...
    if (t >= 1 && test1Audio1.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    if (t >= 1 && test1Audio1Sound.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    if (t >= 2 && test1Audio1Mute.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    frameRemains = 2 + 18 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (test1Audio1Mute.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      test1Audio1Mute.setAutoDraw(false);
    }
    // start/stop test1Audio2
    if (t >= 3 && test1Audio2.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    if (t >= 1 && test1Audio2Mute1.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    if (t >= 3 && test1Audio2Sound.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    if (t >= 4 && test1Audio2Mute2.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    frameRemains = 4 + 16 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (test1Audio2Mute2.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      test1Audio2Mute2.setAutoDraw(false);
    }
    
    // *test1Response* updates
    if (t >= 4 && test1Response.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    // *test1F* updates
    if (t >= 4 && test1F.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
    ...
    // *test1J* updates
    if (t >= 4 && test1J.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
...
var _skipSurveyInstr_allKeys;
var surveyInstructionsComponents;
function surveyInstructionsRoutineBegin(trials) {
  return function () {
    //------Prepare to start Routine 'surveyInstructions'-------
    t = 0;
    surveyInstructionsClock.reset(); // clock
    frameN = -1;
    routineTimer.add(15.000000);
    // update component parameters for each repeat
    skipSurveyInstr.keys = undefined;
    skipSurveyInstr.rt = undefined;
    _skipSurveyInstr_allKeys = [];
    // keep track of which components have finished
    surveyInstructionsComponents = [];
    surveyInstructionsComponents.push(surveyInstrText);
    surveyInstructionsComponents.push(skipSurveyInstr);
    
    for (const thisComponent of surveyInstructionsComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function surveyInstructionsRoutineEachFrame(trials) {
  return function () {
    //------Loop for each frame of Routine 'surveyInstructions'-------
    let continueRoutine = true; // until we're told otherwise
    // get current time
    ...
    frameRemains = 0.0 + 15 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (surveyInstrText.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      surveyInstrText.setAutoDraw(false);
    }
    
    // *skipSurveyInstr* updates
    if (t >= 0.0 && skipSurveyInstr.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      skipSurveyInstr.tStart = t;  // (not accounting for frame time here)
      skipSurveyInstr.frameNStart = frameN;  // exact frame index
      
      // keyboard checking is just starting
      psychoJS.window.callOnFlip(function() { skipSurveyInstr.clock.reset(); });  // t=0 on next screen flip
      psychoJS.window.callOnFlip(function() { skipSurveyInstr.start(); }); // start on screen flip
      psychoJS.window.callOnFlip(function() { skipSurveyInstr.clearEvents(); });
    }

    frameRemains = 0.0 + 15 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (skipSurveyInstr.status === PsychoJS.Status.STARTED && t >= frameRemains) {
      skipSurveyInstr.status = PsychoJS.Status.FINISHED;
  }

    if (skipSurveyInstr.status === PsychoJS.Status.STARTED) {
      let theseKeys = skipSurveyInstr.getKeys({keyList: [], waitRelease: false});
      _skipSurveyInstr_allKeys = _skipSurveyInstr_allKeys.concat(theseKeys);
      if (_skipSurveyInstr_allKeys.length > 0) {
        skipSurveyInstr.keys = _skipSurveyInstr_allKeys[_skipSurveyInstr_allKeys.length - 1].name;  // just the last key pressed
        skipSurveyInstr.rt = _skipSurveyInstr_allKeys[_skipSurveyInstr_allKeys.length - 1].rt;
        // a response ends the routine
        continueRoutine = false;
      }
    }
...
function surveyInstructionsRoutineEnd(trials) {
  return function () {
    ...
    psychoJS.experiment.addData('skipSurveyInstr.keys', skipSurveyInstr.keys);
    if (typeof skipSurveyInstr.keys !== 'undefined') {  // we had a response
        psychoJS.experiment.addData('skipSurveyInstr.rt', skipSurveyInstr.rt);
        routineTimer.reset();
        }
    
    skipSurveyInstr.stop();
    return Scheduler.Event.NEXT;
  };
}
...
var typedWords;
var thisWord;
var ageSurveyComponents;
function ageSurveyRoutineBegin(trials) {
  return function () {
    //------Prepare to start Routine 'ageSurvey'-------
    t = 0;
    ageSurveyClock.reset(); // clock
    frameN = -1;
    // update component parameters for each repeat
    ...
    // keep track of which components have finished
    ageSurveyComponents = [];
    ageSurveyComponents.push(age);
    ageSurveyComponents.push(ageInstr);
    
    for (const thisComponent of ageSurveyComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
var _pj;
var keys;
function ageSurveyRoutineEachFrame(trials) {
  return function () {
    //------Loop for each frame of Routine 'ageSurvey'-------
    let continueRoutine = true; // until we're told otherwise
    // get current time
    ...
    if (((displayText !== oldDisplayText) || (cursorCounter === 1))) {
    ...
    // *ageInstr* updates
    if (t >= 0.0 && ageInstr.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      ageInstr.tStart = t;  // (not accounting for frame time here)
...
function ageSurveyRoutineEnd(trials) {
  return function () {
...
var genderSurveyComponents;
function genderSurveyRoutineBegin(trials) {
  return function () {
    //------Prepare to start Routine 'genderSurvey'-------
    t = 0;
    genderSurveyClock.reset(); // clock
    frameN = -1;
    // update component parameters for each repeat
    kb = new keyboard.Keyboard();
    textStarted = 0;
    ...
    // keep track of which components have finished
    genderSurveyComponents = [];
    genderSurveyComponents.push(gender);
    genderSurveyComponents.push(genderInstr);
    
    for (const thisComponent of genderSurveyComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function genderSurveyRoutineEachFrame(trials) {
  return function () {
    //------Loop for each frame of Routine 'genderSurvey'-------
    let continueRoutine = true; // until we're told otherwise
    // get current time
    ...
    // *genderInstr* updates
    if (t >= 0.0 && genderInstr.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      genderInstr.tStart = t;  // (not accounting for frame time here)
...
function genderSurveyRoutineEnd(trials) {
  return function () {
...
var nativeLangSurveyComponents;
function nativeLangSurveyRoutineBegin(trials) {
  return function () {
    //------Prepare to start Routine 'nativeLangSurvey'-------
    t = 0;
    nativeLangSurveyClock.reset(); // clock
    frameN = -1;
    // update component parameters for each repeat
    kb = new keyboard.Keyboard();
    textStarted = 0;
    ...
    // keep track of which components have finished
    nativeLangSurveyComponents = [];
    nativeLangSurveyComponents.push(nativeLang);
    nativeLangSurveyComponents.push(nativeLangInstr);
    
    for (const thisComponent of nativeLangSurveyComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function nativeLangSurveyRoutineEachFrame(trials) {
  return function () {
    //------Loop for each frame of Routine 'nativeLangSurvey'-------
    let continueRoutine = true; // until we're told otherwise
    // get current time
    ...
    // *nativeLang* updates
    if (t >= 0.0 && nativeLang.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      nativeLang.tStart = t;  // (not accounting for frame time here)
    ...
    if (t >= 0.0 && nativeLangInstr.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      nativeLangInstr.tStart = t;  // (not accounting for frame time here)
...
function nativeLangSurveyRoutineEnd(trials) {
  return function () {
...
var otherLangSurveyComponents;
function otherLangSurveyRoutineBegin(trials) {
  return function () {
    //------Prepare to start Routine 'otherLangSurvey'-------
    t = 0;
    otherLangSurveyClock.reset(); // clock
    frameN = -1;
    // update component parameters for each repeat
    kb = new keyboard.Keyboard();
    textStarted = 0;
    ...
    // keep track of which components have finished
    otherLangSurveyComponents = [];
    otherLangSurveyComponents.push(otherLang);
    otherLangSurveyComponents.push(otherLangInstr);
    
    for (const thisComponent of otherLangSurveyComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
...
function otherLangSurveyRoutineEachFrame(trials) {
  return function () {
    //------Loop for each frame of Routine 'otherLangSurvey'-------
    let continueRoutine = true; // until we're told otherwise
    // get current time
    ...
    // *otherLang* updates
    if (t >= 0.0 && otherLang.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      otherLang.tStart = t;  // (not accounting for frame time here)
    ...
    if (t >= 0.0 && otherLangInstr.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      otherLangInstr.tStart = t;  // (not accounting for frame time here)
...
function otherLangSurveyRoutineEnd(trials) {
  return function () {
...
function thankuserRoutineBegin(trials) {
  return function () {
    ...
    // keep track of which components have finished
    thankuserComponents = [];
    thankuserComponents.push(byeText);
...
function thankuserRoutineEachFrame(trials) {
  return function () {
    ...
    // *byeText* updates
    if (t >= 0.0 && byeText.status === PsychoJS.Status.NOT_STARTED) {
      // keep track of start time/frame for later
      byeText.tStart = t;  // (not accounting for frame time here)
    ...
    frameRemains = 0.0 + 10 - psychoJS.window.monitorFramePeriod * 0.75;  // most of one frame period left
    if (byeText.status === PsychoJS.Status.STARTED && t >= frameRemains) {
